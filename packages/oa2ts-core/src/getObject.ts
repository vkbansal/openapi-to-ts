...
export default function getObject(
  ...
  if (item.allOf) {
    ...
    return transformType(mappedValues, typeNode =>
      ts.factory.createIntersectionTypeNode(typeNode)
    );
  }
  ...
  if (item.oneOf) {
    ...
    return transformType(mappedValues, typeNode =>
      ts.factory.createUnionTypeNode(typeNode)
    );
  }

  return transformType(getObjectProperties(item), typeNode =>
    ts.factory.createTypeLiteralNode(typeNode)
  );
}
