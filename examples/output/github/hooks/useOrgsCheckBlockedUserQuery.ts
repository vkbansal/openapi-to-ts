/* eslint-disable */
// This code is autogenerated using @oa2ts/cli.
// Please do not modify this code directly.
import { useQuery } from '@tanstack/react-query';

export interface UseOrgsCheckBlockedUserQueryPathParams {
	org: string;
	username: string;
}

export interface UseOrgsCheckBlockedUserQueryProps extends UseOrgsCheckBlockedUserQueryPathParams {}

/**
 *
 */

export function useOrgsCheckBlockedUserQuery(props: UseOrgsCheckBlockedUserQueryProps) {
	const { org, username } = props;

	return useQuery(['orgs/check-blocked-user', org, username]);
}
